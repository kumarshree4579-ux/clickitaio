      ...
      {items.map((item, i) => (
        <div key={i}
          ...
          <button
            ...
            <span className={`font-semibold text-sm sm:text-base leading-snug ${open === i ? 'text-primary-dark' : 'text-gray-800'}`}>
              {item.q}
            </span>
            <span className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-colors ${open === i ? 'bg-primary-light text-primary' : 'bg-gray-100 text-gray-400'}`}>
