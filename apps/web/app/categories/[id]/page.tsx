...
export default async function CategoryDetailPage({ 
  ...
  return (
    <>
      ...
      <main className="w-full max-w-[1600px] mx-auto bg-gray-50 flex flex-row" style={{ minHeight: 'calc(100vh - 96px)' }}>
        
        {/* Subcategory Sidebar (25% on mobile) */}
        <aside className="w-1/4 lg:w-1/5 bg-white border-r border-gray-100 shrink-0 overflow-y-auto scrollbar-hide" style={{ maxHeight: 'calc(100vh - 96px)' }}>
          <div className="py-2">
            <Link 
              href={`/categories/${mainCategoryId}`}
              className={`flex flex-col items-center justify-center p-2 sm:px-5 sm:py-4 text-xs sm:text-sm font-medium border-l-[3px] sm:border-l-4 transition-colors ${
                !activeSubId 
                  ? 'border-[var(--color-primary)] bg-indigo-50/50 text-[var(--color-primary)]' 
                  : 'border-transparent text-gray-600 hover:bg-gray-50'
              }`}
            >
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center bg-gray-100 mb-2 border border-gray-200">
        ...
        <div className="flex-1 w-3/4 lg:w-4/5 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 96px)' }}>
