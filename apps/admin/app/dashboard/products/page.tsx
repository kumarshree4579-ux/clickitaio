...
export default function ProductsPage() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      const data = await res.json();
      exportToCSV('products_export', data.items || []);
    } catch (err) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={handleExport} className="bg-white border border-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-50 flex items-center gap-2">
