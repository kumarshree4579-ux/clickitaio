...
router.get('/summary', requireAuth, requireRole('super_admin', 'order_manager'), async (_req, res: Response) => {
  ...
  const [
    ...
    totalOrders, activeOrders, pendingOrders, deliveredOrders, cancelledOrders,
    ...
    Order.countDocuments({ status: { $nin: ['cancelled', 'completed', 'delivered', 'refunded', 'returned', 'abandoned'] } }),
  ...
  return res.json({
    ...
    orders: { total: totalOrders, active: activeOrders, pending: pendingOrders, delivered: deliveredOrders, cancelled: cancelledOrders },
